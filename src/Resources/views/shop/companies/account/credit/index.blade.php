...
<x-shop::layouts.account>
    ...
    <div class="mx-4 flex-auto max-md:mx-6 max-sm:mx-4">
        ...
        {!! view_render_event('bagisto.shop.companies.account.credit.before', ['credit' => $credit]) !!}

        <!-- Summary cards (ordering mirrors the admin panel) -->
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <!-- Row 1: Outstanding balance + Available credit -->
            <div class="rounded-xl border border-zinc-200 bg-white p-5">
                <span class="text-xs uppercase tracking-wide text-zinc-400">@lang('b2b::app.shop.companies.account.credit.outstanding-balance')</span>
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-5">
                <span class="text-xs uppercase tracking-wide text-zinc-400">@lang('b2b::app.shop.companies.account.credit.available-credit')</span>
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-5">
                <span class="text-xs uppercase tracking-wide text-zinc-400">@lang('b2b::app.shop.companies.account.credit.credit-limit')</span>
                <p class="mt-1.5 text-xl font-semibold text-zinc-800">{{ core()->formatBasePrice($credit->credit_limit) }}</p>
            </div>

            <div class="rounded-xl border border-zinc-200 bg-white p-5">
                <span class="text-xs uppercase tracking-wide text-zinc-400">@lang('b2b::app.shop.companies.account.credit.status')</span>
                <p class="mt-1.5">
                    <span class="{{ $credit->status ? 'label-active' : 'label-canceled' }}">
                        @lang('b2b::app.shop.companies.account.credit.'.($credit->status ? 'enabled' : 'disabled'))
                    </span>
                </p>
            </div>
        </div>
        ...
        <!-- Credit history -->
        <p class="mb-3 mt-8 text-lg font-medium text-zinc-800">@lang('b2b::app.shop.companies.account.credit.history')</p>

        <x-shop::datagrid :src="route('shop.customers.account.credit.index')" />

        {!! view_render_event('bagisto.shop.companies.account.credit.after', ['credit' => $credit]) !!}
    </div>
</x-shop::layouts.account>
